...
app.delete('/api/users/:userId', authenticate, async (req, res) => {
  try {
    ...
    for (const beat of userBeats) {
      ...
      // Delete all vocal versions of this beat
    ...
    // Remove user's beats from other users' data
    for (const beat of userBeats) {
      for (const u of users) {
        if (u.uploadedBeats) u.uploadedBeats = u.uploadedBeats.filter(id => id !== beat.id);
        if (u.recordings) {
          for (const version of beat.versions) {
            u.recordings = u.recordings.filter(id => id !== version.id);
          }
        }
      }
    }
    ...
    // Filter out user's beats from beats.json
    let updatedBeats = beats.filter(b => b.producerId !== user.id);
    // Also remove recordings from remaining beats where user was vocalist
...
app.delete('/api/beats/:beatId', authenticate, (req, res) => {
  try {
    ...
    // Delete all vocal version files
    for (const version of beat.versions) {
      const vocalPath = path.join(__dirname, version.fileUrl);
      if (fs.existsSync(vocalPath)) {
        fs.unlinkSync(vocalPath);
    ...
    // Remove all votes for recordings in this beat
    let votes = readData('votes.json');
...
// DELETE recording (vocal version)
  ...
  try {
    ...
    for (let i = 0; i < beats.length; i++) {
      ...
      if (versionIndex !== -1) {
        ...
        // Remove votes for this recording
        let votes = readData('votes.json');
...
app.post('/api/beats/:beatId/record', authenticate, upload.single('vocal'), async (req, res) => {
  try {
    ...
    const { title, description, tags, trackData } = req.body;
    const beat = beats[beatIndex];
    ...
    const newVersion = {
      ...
      filename: req.file.filename,
      trackData: trackData || null, // Store multi-track data
...
app.listen(PORT, () => {
  ...
  console.log('\n✨ Features:');
  console.log('  • 8-Track recording studio');
  console.log('  • Download beats');
  console.log('  • Delete your content');
  console.log('  • Search users & music');
  console.log('  • Social following');
  console.log('  • Voting & leaderboards\n');
});